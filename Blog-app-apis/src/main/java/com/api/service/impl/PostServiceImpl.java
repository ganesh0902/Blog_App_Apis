...
public class PostServiceImpl implements PostService{
	...
	public PostDto updatePost(PostDto post, int postId) {
		...
		try {
			...
			updatedPost.setImageName(map2.getImageName());
	...
	public List<PostDto> searchByPosts(String keyword) {
		...
		List<PostDto> collect = searchByTitle.stream().map(post->this.modelMapper.map(post,PostDto.class)).collect(Collectors.toList());
			
		//searchByTitle.stream().map(post->post.getTitle()).forEach(System.out::println);
